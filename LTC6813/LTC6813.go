...
func (this *LTC6813) checkPEC(sError string, bCheckCmd bool) error {
	...
	for i := 0; i < this.chainLength; i++ {
		if this.getDataPEC(i) != this.calculateDataPEC(i) {
			...
			return fmt.Errorf("PEC error in data block %d %x [%x] ", i, this.getData(0), this.getData(i))
		}
	}
	return nil
}
...
func (this *LTC6813) MeasureVoltagesAndAux() (int, error) {
	...
	this.setTemperatureSensor(this.temperatureSensor)
	return i, err
}
...
*/
func calculateTemperature(t uint16) (float64, error) {
	if (t > 28000) || (t < 100) {
		return -273.15, fmt.Errorf("ERR")
	} else {
		return (1.0 / ((math.Log(1/((30000.0/float64(t))-1)) / BCOEFFICIENT) + (0.003354))) - 273.15, nil
	}
}
...
func (this *LTC6813) GetVoltagesAsJSON() string {
	...
	defer this.mu.Unlock()
	s := `"voltages":[`
	for _, bank := range this.readings {
		for _, cell := range bank.CellVolts {
			s += fmt.Sprintf("%1.4f,", float32(cell)/10000)
		}
	}
	return s + "]"
}
...
func (this *LTC6813) GetTemperaturesAsJSON() string {
	...
	defer this.mu.Unlock()
	for _, bank := range this.readings {
		s, err := json.Marshal(bank.temperatures)
		if err == nil {
			retString += string(s)
			retString += ","
		}
...
func (this *LTC6813) WriteI2CByte(bank int, address uint8, command uint8, data uint8) (int, error) {
	...
	defer this.mu.Unlock()
	this.clearPacket()
	this.setData(0, I2CStart+(address>>4), ((address << 4) & 0xf0), ((command >> 4) & 0x0f), ((command << 4) & 0xf0), ((data >> 4) & 0x0f), ((data<<4)&0xf0)+I2CNackStop)
...
func (this *LTC6813) WriteI2CWord(address uint8, command uint8, data uint16) (int, error) {
	...
	defer this.mu.Unlock()
	this.clearPacket()
	this.setData(0, I2CStart+(address>>4), ((address << 4) & 0xf0), (command >> 4), (command << 4), byte(data>>4), byte(data<<4)+I2CNackStop)
	this.setData(1, I2CStart+(address>>4), ((address << 4) & 0xf0), (command >> 4), (command << 4), byte(data>>4), byte(data<<4)+I2CNackStop)
	this.setData(2, I2CStart+(address>>4), ((address << 4) & 0xf0), (command >> 4), (command << 4), byte(data>>4), byte(data<<4)+I2CNackStop)
	this.setCommand(WRCOMM)
	this.sendCommand()
	//	this.clearPacket()
	//	this.setCommand(STCOMM)
...
func (this *LTC6813) ReadI2CByte(bank int, address uint8, command uint8) (string, error) {
	...
	defer this.mu.Unlock()
	this.clearPacket()
	this.setData(0, I2CStart+(address>>4), ((address<<4)&0xf0)+I2CWRITE, (command >> 4), (command << 4), I2CNoTransmit, 0)
	this.setData(1, I2CStart+(address>>4), ((address<<4)&0xf0)+I2CWRITE, (command >> 4), (command << 4), I2CNoTransmit, 0)
	this.setData(2, I2CStart+(address>>4), ((address<<4)&0xf0)+I2CWRITE, (command >> 4), (command << 4), I2CNoTransmit, 0)
	this.setCommand(WRCOMM)
	this.sendCommand()
	...
	this.clearPacket()
	this.setData(0, I2CStart+(address>>4), ((address<<4)&0xf0)+I2CREAD, 0x0F, 0x0F+I2CNackStop, I2CNoTransmit, 0)
	this.setData(1, I2CStart+(address>>4), ((address<<4)&0xf0)+I2CREAD, 0x0F, 0xF0+I2CNackStop, I2CNoTransmit, 0)
	this.setData(2, I2CStart+(address>>4), ((address<<4)&0xf0)+I2CREAD, 0x0F, 0xF0+I2CNackStop, I2CNoTransmit, 0)
	this.setCommand(WRCOMM)
	this.sendCommand()
	...
	//	this.setData( 1, 0,0,0,0,0,0)
	//	this.setData( 2, 0,0,0,0,0,0)
	this.sendCommand()
...
func (this *LTC6813) ReadI2CWord(bank int, address uint8, command uint8) (string, error) {
	...
	this.clearPacket()
	this.setData(0, I2CStart+(address>>4), ((address<<4)&0xf0)+I2CREAD, 0x0F, 0xF0+I2CACK, 0x0F, 0xF0+I2CNackStop)
	this.setData(1, I2CStart+(address>>4), ((address<<4)&0xf0)+I2CREAD, 0x0F, 0xF0+I2CACK, 0x0F, 0xF0+I2CNackStop)
	this.setData(2, I2CStart+(address>>4), ((address<<4)&0xf0)+I2CREAD, 0x0F, 0xF0+I2CACK, 0x0F, 0xF0+I2CNackStop)
	this.setCommand(WRCOMM)
	this.sendCommand()
...
*/
func (this *LTC6813) ReadI2CWordData(bank int, address uint8, command uint8) (int, error) {
	this.mu.Lock()
	defer this.mu.Unlock()
	this.clearPacket()
	this.setData(0, I2CStart+(address>>4), ((address<<4)&0xf0)+I2CWRITE, (command >> 4), (command << 4), I2CNoTransmit, 0)
	...
	if err != nil {
		...
		fmt.Println("Data = ", this.getData(0))
		msb := this.getByte(bank, 1)
		lsb := this.getByte(bank, 2)
		fmt.Println("msb=", msb, "\nlsb=", lsb, "\n")
		return (int(msb) * 256) + int(lsb), nil
	}
}
...
func (this *LTC6813) GetI2CTemp(sensor int) (float64, error) {
	...
	if err != nil {
		...
		return ((510 * (float64(v) / 65535)) - 273.15), nil
	}
}
